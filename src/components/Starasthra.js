...
function Starasthra() {
    ...
    firebase.firestore().collection('Registration Number Starofasthra').doc('unique').get().then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        firebase.firestore().collection('Registration Number Starofasthra').doc('unique').update({
            number: regNo + 1
        })

        firebase.firestore().collection('Contestant Starofasthra').add(
            {
                "Name": name,
                "Phone no.": phone,
                "Email": email,
